...
from baselines import positions, nominal

PRED_DT = 0.04 # prediction sampling period/timestep
PLAN_DT = 0.4
N_HUMAN_JOINTS = 11
PLAN_TIME = 0.6 # time to plan in sec
...
# WEIGHTS = [0., 0., 0., 70., 0., 0.]
WEIGHTS = [1.5, 0.1, 0.09, 25., 8, 6]
GOAL_THRESH = 0.1
...
def is_same_dir(x, y):
    ...
    return cos_theta >= np.cos(np.pi/6) # at most 30 degrees off
    
def is_between(x, endpts):
    """
    Returns True if the query point x is between points 1 and 2 
    All points should be np vectors of any size
    """
    l, r = endpts
    x = np.asarray(x)
...
def get_nom_traj(goal, start, n_ts):
    """
    This implementation returns a nominal trajectory that takes a roughly
    linear cartesian path from start to goal 
    """
    start_cart, goal_cart = jaco_ee_fk(start), jaco_ee_fk(goal)
    # generate a favorable midpoint
    if np.linalg.norm(goal_cart - start_cart) > 0.25:
        # print(start_cart)
        cart_midpoint = 0.5*(start_cart + goal_cart)
        cart_midpoint[2] = max(cart_midpoint[2] + 0.05, 0.4)
        cart_midpoint[1] = min(cart_midpoint[1] + 0.3, -0.15)
        cart_traj = np.array([start_cart, cart_midpoint, goal_cart])
    else:
        cart_traj = np.array([start_cart, goal_cart])

    nom_highsmp, nom_times = nominal.cart2jnt(cart_traj, start, goal, rot_ctrl=True)

    # downsample new trajectory
    x_in = nom_times
    x_out = PLAN_DT*np.arange(n_ts)
    x_interp = x_out[(x_out < x_in[-1]).nonzero()[0]]
    nom_downsmp = np.concatenate([
        np.interp(x_interp, x_in, nom_highsmp[:,i]).reshape((-1, 1))
        for i in range(nom_highsmp.shape[1])
    ], axis=1)
    if x_interp.size < x_out.size:
        nom_downsmp = np.concatenate((nom_downsmp, 
            np.tile(nom_downsmp[-1,:], (x_out.size - x_interp.size)).reshape(
            (-1, nom_downsmp.shape[1]))),
            axis=0)
    return nom_downsmp
...
gen3_ee_name = "EndEffector_Link"
gen3_chain = kp.build_serial_chain_from_urdf(open(urdf_path).read(), gen3_ee_name)
...
def jaco_ee_fk(posn):
    ...
    """
    fk = gen3_chain.forward_kinematics(posn)
    return np.array(fk.pos)
...
class TrajectoryManager:
    def __init__(self, goal, weights=WEIGHTS, warm_start=False):
        self.goal = np.asarray(goal)
        self.cart_goal = jaco_ee_fk(self.goal)
        self.weights = weights
        self.solve_srv = rospy.ServiceProxy("/comoto", Solve)
        ...
        self.is_warm_start = warm_start

        self.is_torn_down = False
        self.pub_subs = [
            rospy.Subscriber("/right_arm/base_feedback/joint_state", JointState, self.joint_state_cb),
            rospy.Subscriber("/prediction_traj", JointTrajectory, self.joint_pred_cb),
            self.traj_pub
        ]

        if self.is_warm_start:
            # replan slowly to keep CoMOTO warm
            self.timers = [
                rospy.Timer(rospy.Duration(1), self.mpc_replan)
            ]
        else:
            # replan as fast as possible (replan exits if another replan is happening)
            self.timers = [
                rospy.Timer(rospy.Duration(0.2), self.mpc_replan)
            ]
    ...
    def mpc_replan(self, evt):
        ...
        """
        if self.reached_goal and not self.is_warm_start:
            self.teardown()
            return True
        ...
        if len(resp.solve_traj.points) == 0:
            self.solve_mut.release()
            return False

        out_traj = [np.array(x.positions) for x in resp.solve_traj.points]
        out_times = [x.time_from_start.to_sec() for x in resp.solve_traj.points]
        
        # if out_times[0] < 0.75*PLAN_DT:
        #     out_traj = out_traj[1:]
        #     out_times = out_times[1:]
        
        while len(out_times) > 1 and \
            (is_same_dir(curr_posn - out_traj[0], out_traj[1] - out_traj[0]) or \
            np.linalg.norm(curr_posn - out_traj[0]) < 0.1):
            out_traj = out_traj[1:]
            out_times = out_times[1:]
        
        out_traj = [curr_posn] + out_traj
        if len(out_traj) > 3:
            np_out_traj = np.array(out_traj)
            out_flt = savgol_filter(np_out_traj, 3, 2, axis=0)
        else:
            out_flt = np.array(out_traj)
            
        out_msg = JointTrajectory()
        out_times = 1.1*np.array(out_times)
        for i in range(len(out_times)):
            pt = JointTrajectoryPoint()
            pt.time_from_start = rospy.Duration(out_times[i])
            pt.positions = out_flt[i+1,:]
            out_msg.points.append(pt)
            
        if not self.is_warm_start:
            self.traj_pub.publish(out_msg)
...
if __name__ == "__main__":
    ...
    goal_idx = int(sys.argv[1])
    goal = positions.posns[goal_idx]
    manager = TrajectoryManager(goal)
    rospy.spin()